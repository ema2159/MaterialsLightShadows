(function (global, factory) {
	...
	var R = {
		...
		needReZone: true,
		isEventsInit: false,
		prevDefault: ['contextmenu', 'pointerdown', 'pointermove', 'pointerup'],
		...
		initEvents: function initEvents() {
			...
			window.addEventListener('resize', R.resize, false); //window.addEventListener( 'pointerdown', R, false );
		...
		handleEvent: function handleEvent(event) {
			...
			if (R.isMobile) {
				...
				if (event.type === 'touchstart') e.type = 'pointerdown';
				if (event.type === 'touchend') e.type = 'pointerup';
				if (event.type === 'touchmove') e.type = 'pointermove';
			}
			/*
			if( event.type === 'touchstart'){ e.type = 'pointerdown'; R.findID( e ); }
			if( event.type === 'touchend'){ e.type = 'pointerup';	if( R.ID !== null ) R.ID.handleEvent( e ); R.clearOldID(); }
			if( event.type === 'touchmove'){ e.type = 'pointermove';	}
			*/


			if (e.type === 'pointerdown') R.lock = true;
			if (e.type === 'pointerup') R.lock = false;
			if (R.isMobile && e.type === 'pointerdown') R.findID(e);
			if (e.type === 'pointermove' && !R.lock) R.findID(e);
			...
			if (R.isMobile && e.type === 'pointerup') R.clearOldID();
		},
		// ----------------------
		//	 ID
		// ----------------------
	...
	var Proto = /*#__PURE__*/function () {
		...
		_proto.pointerdown = function pointerdown(e) {
			return false;
		};

		_proto.pointermove = function pointermove(e) {
			return false;
		};

		_proto.pointerup = function pointerup(e) {
			return false;
		};
	...
	var Button = /*#__PURE__*/function (_Proto) {
		...
		_proto.pointerdown = function pointerdown(e) {
			...
			return this.pointermove(e); // true;
		};
		...
		_proto.initLoader = function initLoader() {
			...
			}.bind(this), false); //this.c[3].addEventListener( 'pointerdown', function(e){	}.bind(this), false );
			//this.c[2].events = [	];
			//this.c[3].events = [ 'change', 'mouseover', 'pointerdown', 'pointerup', 'mouseout' ];
	...
	var Group = /*#__PURE__*/function (_Proto) {
		...
		_proto.handleEvent = function handleEvent(e) {
			...
			switch (name) {
				case 'content':
					this.cursor();
					if (Roots.isMobile && type === 'pointerdown') this.getNext(e, change);
					if (this.target) targetChange = this.target.handleEvent(e); //if( type === 'pointermove' ) change = this.styles('def');
	...
	var Gui = /*#__PURE__*/function () {
		...
		_proto.handleEvent = function handleEvent(e) {
			...
			var targetChange = false;
			var name = this.testZone(e);
			if (type === 'pointerup' && this.isDown) this.isDown = false;
			if (type === 'pointerdown' && !this.isDown) this.isDown = true;
			...
			switch (name) {
				case 'content':
					...
					if (type === 'pointermove') change = this.mode('def');
				...
				case 'bottom':
					this.clearTarget();
					if (type === 'pointermove') change = this.mode('bottomOver');
				...
				case 'scroll':
					this.clearTarget();
					if (type === 'pointermove') change = this.mode('scrollOver');
					if (type === 'pointerdown') change = this.mode('scrollDown');
